...
describe("runTeardown", () => {
	beforeEach(() => {
		...
		mkdirSync(join(MAIN_REPO, ".caspian"), { recursive: true });
	...
	test("returns success when config has no teardown commands", async () => {
		writeFileSync(
			join(MAIN_REPO, ".caspian", "config.json"),
	...
	test("ignores config in worktreePath", async () => {
		...
		mkdirSync(join(WORKTREE, ".caspian"), { recursive: true });
		writeFileSync(
			join(WORKTREE, ".caspian", "config.json"),
	...
	test("sets environment variables for teardown scripts", async () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				teardown: [
					`echo "$CASPIAN_WORKSPACE_NAME|$CASPIAN_ROOT_PATH" > "${envFile}"`,
				],
			}),
		);
