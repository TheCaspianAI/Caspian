...
export function OpenInButton({ path, label, showShortcuts = false }: OpenInButtonProps) {
	...
	const showCopyPathShortcut = showShortcuts && copyPathShortcut !== "Unassigned";
		...
		<ButtonGroup>
			...
			<DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
				<DropdownMenuTrigger asChild>
					<Button variant="outline" size="sm" className="gap-1" disabled={!path}>
						<span>Open</span>
				...
				<DropdownMenuContent align="end" className="w-48">
					{APP_OPTIONS.map((app) => (
						...
						>
							<div className="flex items-center gap-2">
								<img src={app.icon} alt={app.label} className="size-4 object-contain" />
								<span>{app.label}</span>
							</div>
					...
					<DropdownMenuItem onClick={handleCopyPath} className="flex items-center justify-between">
						...
						{showCopyPathShortcut && (
							<span className="text-xs text-muted-foreground">{copyPathShortcut}</span>
						)}
					</DropdownMenuItem>
				</DropdownMenuContent>
			</DropdownMenu>
