...
import { useNodeShortcuts } from "renderer/hooks/useNodeShortcuts";
import { PortsList } from "./PortsList";
import { RepositorySection } from "./RepositorySection";
import { SidebarDropZone } from "./SidebarDropZone";
import { NodeSidebarFooter } from "./NodeSidebarFooter";
...
export function NodeSidebar({
	...
	const { groups } = useNodeShortcuts();

	// Calculate shortcut base indices for each repository group using cumulative offsets
	const repositoryShortcutIndices = useMemo(
		() =>
			groups.reduce<{ indices: number[]; cumulative: number }>(
				(acc, group) => ({
					indices: [...acc.indices, acc.cumulative],
					cumulative: acc.cumulative + group.nodes.length,
				}),
	...
	return (
		<SidebarDropZone className="flex flex-col h-full glass rounded-xl overflow-hidden">
			<NodeSidebarHeader isCollapsed={isCollapsed} />
			...
			<div className="flex-1 overflow-y-auto hide-scrollbar fade-mask-y">
				{groups.map((group, index) => (
					<RepositorySection
						...
						githubOwner={group.repository.githubOwner}
						mainRepoPath={group.repository.mainRepoPath}
						nodes={group.nodes}
						shortcutBaseIndex={repositoryShortcutIndices[index]}
						index={index}
				...
				{groups.length === 0 && !isCollapsed && (
					<div className="flex flex-col items-center justify-center py-8 px-4">
						...
						<span className="text-xs text-muted-foreground/60 text-center leading-relaxed max-w-[180px]">
							Open a repository or drag a Git repository here to get started
						</span>
					</div>
				)}
			</div>
			...
			<NodeSidebarFooter isCollapsed={isCollapsed} />
		</SidebarDropZone>
	);
}
