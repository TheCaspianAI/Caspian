...
interface NodeHoverCardContentProps {
	nodeId: string;
	nodeAlias?: string;
}

export function NodeHoverCardContent({
	nodeId,
	nodeAlias,
}: NodeHoverCardContentProps) {
	const { data: worktreeInfo } =
		electronTrpc.nodes.getWorktreeInfo.useQuery(
			{ nodeId },
			{ enabled: !!nodeId },
		);
	...
	const hasCustomAlias =
		nodeAlias && worktreeName && nodeAlias !== worktreeName;
	...
	return (
		<div className="space-y-3">
			...
			<div className="space-y-1.5">
				{hasCustomAlias && (
					<div className="text-sm font-medium">{nodeAlias}</div>
				)}
				{worktreeName && (
