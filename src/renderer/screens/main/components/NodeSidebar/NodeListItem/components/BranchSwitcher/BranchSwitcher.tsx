...
export function BranchSwitcher({
	...
	const { data: branchesData, isLoading } =
		electronTrpc.nodes.getBranches.useQuery(
			{ repositoryId, fetch: false },
			{ enabled: isOpen },
		);

	const switchBranch =
		electronTrpc.nodes.switchBranchNode.useMutation({
			onSuccess: () => {
				utils.nodes.invalidate();
			},
		});

	// Branches in use by worktrees (branch -> nodeId)
	const inUseNodes = useMemo(() => {
		return branchesData?.inUseNodes ?? {};
	}, [branchesData]);

	// Set of branch names in use for quick lookup
	const inUseBranches = useMemo(() => {
		return new Set(Object.keys(inUseNodes));
	}, [inUseNodes]);
	...
	const handleBranchClick = (branch: string) => {
		...
		// If branch is in use by a worktree, jump to that node
		const worktreeNodeId = inUseNodes[branch];
		if (worktreeNodeId) {
			navigateToNode(worktreeNodeId, navigate);
			setIsOpen(false);
			return;
		}

		// Otherwise switch this node to the new branch
		toast.promise(switchBranch.mutateAsync({ repositoryId, branch }), {
