...
import type { NodePortGroup as NodePortGroupType } from "../hooks/usePortsData";
import { MergedPortBadge } from "./MergedPortBadge";

interface NodePortGroupProps {
	group: NodePortGroupType;
}
		...
		<div>
			<div className="group flex items-center px-3 py-1">
				<button
					...
					className="text-xs truncate text-left transition-colors text-muted-foreground hover:text-foreground cursor-pointer"
				>
					{group.nodeName}
				</button>
