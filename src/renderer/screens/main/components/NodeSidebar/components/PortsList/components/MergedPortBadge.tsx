...
export function MergedPortBadge({ port }: MergedPortBadgeProps) {
	...
	const portNumberColor = port.isActive ? "text-muted-foreground" : "text-muted-foreground/80";

	const displayContent = port.label ? (
		<>
			{port.label} <span className={`font-mono font-normal ${portNumberColor}`}>{port.port}</span>
		</>
	) : (
		...
		<Tooltip>
			...
			<TooltipContent side="top" sideOffset={6}>
				<div className="text-xs space-y-1">
					...
					<div className={`font-mono ${port.label ? "text-muted-foreground" : "font-medium"}`}>
						localhost:{port.port}
					</div>
					{port.isActive && (
						<>
							...
							{canJumpToTerminal && (
								<div className="text-muted-foreground/70 text-[10px]">Click to open node</div>
							)}
						</>
					)}
				</div>
