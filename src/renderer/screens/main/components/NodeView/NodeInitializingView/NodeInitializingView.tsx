...
const DISPLAY_STEPS: NodeInitStep[] = INIT_STEP_ORDER.filter(
	(step) => step !== "pending" && step !== "ready",
);

export function NodeInitializingView({
	nodeId,
	nodeName,
	isInterrupted = false,
}: NodeInitializingViewProps) {
	const progress = useNodeInitProgress(nodeId);
	...
	const retryMutation = electronTrpc.nodes.retryInit.useMutation();
