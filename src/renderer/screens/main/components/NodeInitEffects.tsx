...
export function NodeInitEffects() {
	...
	const pendingTerminalSetups = useNodeInitStore(
		(s) => s.pendingTerminalSetups,
	);
	const removePendingTerminalSetup = useNodeInitStore(
			...
			if (hasSetupScript && hasDefaultPreset && setup.defaultPreset) {
				...
				createOrAttach.mutate(
					...
					},
					{
						...
						onError: (error) => {
							console.error(
								"[NodeInitEffects] Failed to create terminal:",
								error,
							);
			...
			if (progress?.step === "ready") {
				...
				if (setup.defaultPreset === undefined) {
					utils.nodes.getSetupCommands
						...
						.catch((error) => {
							console.error(
								"[NodeInitEffects] Failed to fetch setup commands:",
								error,
							);
