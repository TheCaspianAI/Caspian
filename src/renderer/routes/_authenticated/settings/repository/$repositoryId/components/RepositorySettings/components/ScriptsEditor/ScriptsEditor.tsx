...
export function ScriptsEditor({
	...
	const updateConfigMutation = electronTrpc.config.updateConfig.useMutation({
		onSuccess: () => {
			setHasChanges(false);
			utils.config.getConfigContent.invalidate({ repositoryId });
		},
	});
	...
	return (
		<div className={cn("space-y-6", className)}>
			<div className="flex items-center justify-between">
				<div>
					<p className="text-sm text-muted-foreground">
						Scripts for <span className="font-medium">{repositoryName}</span>
					</p>
				</div>
				<div className="flex gap-2">
					<Button
			...
			<ScriptsSection
				title="Setup Scripts"
				description="Run when a new node is created from this repository"
			...
			<ScriptsSection
				...
				description="Run when a node is deleted"
