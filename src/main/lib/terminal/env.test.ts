...
describe("env", () => {
	...
	describe("buildSafeEnv", () => {
		...
		describe("includes CASPIAN_* prefix vars", () => {
			it("should include CASPIAN_* vars (our metadata)", () => {
				const env = {
					CASPIAN_PANE_ID: "pane-1",
					CASPIAN_TAB_ID: "tab-1",
					CASPIAN_WORKSPACE_ID: "ws-1",
					PATH: "/usr/bin",
				};
				const result = buildSafeEnv(env);
				expect(result.CASPIAN_PANE_ID).toBe("pane-1");
				expect(result.CASPIAN_TAB_ID).toBe("tab-1");
				expect(result.CASPIAN_WORKSPACE_ID).toBe("ws-1");
			});
		});
		...
		describe("Windows platform case-insensitivity", () => {
			...
			it("should include Caspian_* prefix vars case-insensitively on Windows", () => {
				const env = {
					Caspian_Pane_Id: "pane-1",
					CASPIAN_TAB_ID: "tab-1",
					PATH: "/usr/bin",
	...
	describe("buildTerminalEnv", () => {
		...
		describe("terminal metadata", () => {
			it("should set TERM_PROGRAM to Caspian", () => {
				const result = buildTerminalEnv(baseParams);
				expect(result.TERM_PROGRAM).toBe("Caspian");
			});
			...
			it("should set Caspian-specific env vars", () => {
				const result = buildTerminalEnv(baseParams);
			...
			it("should handle optional workspace params", () => {
				...
				expect(result.CASPIAN_WORKSPACE_NAME).toBe("my-workspace");
				expect(result.CASPIAN_WORKSPACE_PATH).toBe("/path/to/workspace");
				expect(result.CASPIAN_ROOT_PATH).toBe("/root/path");
			});

			it("should default optional params to empty string", () => {
				const result = buildTerminalEnv(baseParams);

				expect(result.CASPIAN_WORKSPACE_NAME).toBe("");
				expect(result.CASPIAN_WORKSPACE_PATH).toBe("");
				expect(result.CASPIAN_ROOT_PATH).toBe("");
			});
			...
			it("should include CASPIAN_PORT", () => {
				const result = buildTerminalEnv(baseParams);
				expect(result.CASPIAN_PORT).toBeDefined();
				expect(typeof result.CASPIAN_PORT).toBe("string");
			});
		});

		it("should include CASPIAN_ENV for dev/prod separation", () => {
			const result = buildTerminalEnv(baseParams);
			expect(result.CASPIAN_ENV).toBeDefined();
			expect(["development", "production"]).toContain(result.CASPIAN_ENV);
		});

		it("should include CASPIAN_HOOK_VERSION for protocol versioning", () => {
			const result = buildTerminalEnv(baseParams);
			expect(result.CASPIAN_HOOK_VERSION).toBeDefined();
			expect(result.CASPIAN_HOOK_VERSION).toBe("2");
		});
	});
});
