/**
 * Node Runtime Abstraction Types
 *
 * This module defines the contracts for node-scoped runtime providers.
 * The NodeRuntime boundary encapsulates backend-specific behavior
 ...
 * changes/files/agentEvents to this same boundary for cloud nodes.
 */
 ...
 *
 * This design allows local + cloud nodes to coexist later without
...
export interface NodeRuntimeRegistry {
	 ...
	 * but the interface supports per-node selection for cloud.
	 */
	getForNodeId(nodeId: string): NodeRuntime;

	/**
	 * Get the default runtime (for global/legacy endpoints).
	 * Used by settings screens and endpoints that don't have node context.
	 */
