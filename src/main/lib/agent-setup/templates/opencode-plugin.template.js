// biome-ignore lint/complexity/noUselessLoneBlockStatements: Template marker block for string replacement
{
	MARKER;
}
/**
 * Caspian Notification Plugin for OpenCode
 *
 ...
 * - Debug logging: Set CASPIAN_DEBUG=1 to enable verbose logging
 *
 * SUBAGENT FILTERING:
 ...
 */
export const CaspianNotifyPlugin = async ({ $, client }) => {
	if (globalThis.__caspianOpencodeNotifyPluginV8) return {};
	globalThis.__caspianOpencodeNotifyPluginV8 = true;

	// Only run inside a Caspian terminal session
	if (!process?.env?.CASPIAN_TAB_ID) return {};

	const notifyPath = "{{NOTIFY_PATH}}";
	const debug = process?.env?.CASPIAN_DEBUG === "1";
	...
	const log = (...args) => {
		if (debug) console.log("[caspian-plugin]", ...args);
	};

	/**
	 * Sends a notification to Caspian's notification server.
