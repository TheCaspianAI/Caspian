...
export function createZshWrapper(): void {
	...
	const zprofileScript = `# Caspian zsh profile wrapper
_caspian_home="\${CASPIAN_ORIG_ZDOTDIR:-$HOME}"
[[ -f "$_caspian_home/.zprofile" ]] && source "$_caspian_home/.zprofile"
`;
	...
	const zshrcScript = `# Caspian zsh rc wrapper
_caspian_home="\${CASPIAN_ORIG_ZDOTDIR:-$HOME}"
export ZDOTDIR="$_caspian_home"
[[ -f "$_caspian_home/.zshrc" ]] && source "$_caspian_home/.zshrc"
export PATH="$HOME/${CASPIAN_DIR_NAME}/bin:$PATH"
`;
...
export function createBashWrapper(): void {
	...
	const script = `# Caspian bash rcfile wrapper
...
export function getShellEnv(shell: string): Record<string, string> {
	if (shell.includes("zsh")) {
		return {
			CASPIAN_ORIG_ZDOTDIR: process.env.ZDOTDIR || os.homedir(),
			ZDOTDIR: ZSH_DIR,
		};
	}
