...
export const OPENCODE_PLUGIN_FILE = "caspian-notify.js";

const OPENCODE_PLUGIN_SIGNATURE = "// Caspian opencode plugin";
...
const REAL_BINARY_RESOLVER = `find_real_binary() {
  ...
  for dir in $PATH; do
    ...
    dir="\${dir%/}"
    case "$dir" in
      "$HOME/.caspian/bin"|"$HOME/.caspian-dev/bin") continue ;;
    esac
...
function getMissingBinaryMessage(name: string): string {
	return `Caspian: ${name} not found in PATH. Install it and ensure it is on PATH, then retry.`;
}
...
export function buildClaudeWrapperScript(settingsPath: string): string {
	return `#!/bin/bash
${WRAPPER_MARKER}
# Caspian wrapper for Claude Code
