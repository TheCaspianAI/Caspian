...
class StaticPortsWatcher extends EventEmitter {
	...
	watch(workspaceId: string, worktreePath: string): void {
		...
		// 2. If .caspian dir exists, watch it for ports.json creation
		// 3. If neither exists, watch the worktree root for .caspian creation
		...
		} else if (existsSync(caspianDir)) {
			watchPath = caspianDir;
			watchingFor = "dir";
		...
		try {
			const watcher = watch(watchPath, (_eventType, filename) => {
				...
				if (watchingFor === "dir") {
					// Watching .caspian dir - only care about ports.json
