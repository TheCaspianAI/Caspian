 ...
 * - Socket: ~/.caspian/terminal-host.sock
 * - Auth token: ~/.caspian/terminal-host.token
 */
...
async function startServer(): Promise<void> {
	// Ensure caspian directory exists with proper permissions
	if (!existsSync(CASPIAN_HOME_DIR)) {
		mkdirSync(CASPIAN_HOME_DIR, { recursive: true, mode: 0o700 });
		log("info", `Created directory: ${CASPIAN_HOME_DIR}`);
	}
...
async function main() {
	...
	log("info", `Home directory: ${CASPIAN_HOME_DIR}`);
