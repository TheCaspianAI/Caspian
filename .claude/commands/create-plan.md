# Caspian Implementation Plans (ImplPlans)

> **DO NOT EDIT THIS FILE**
> This file is the ImplPlan template and guide only.
> Create plans in: `plans/<timestamp>-<plan-name>.md`

This document describes the requirements for an implementation plan ("ImplPlan"), a design document that a coding agent can follow to deliver a working feature or system change. Treat the reader as a complete beginner to this repository: they have only the current working tree and the single ImplPlan file you provide. There is no memory of prior plans and no external context.

## Process

Steps:

1. Discovery & Orientation: map the repo, name the scope, enumerate unknowns. Capture initial Assumptions and Open Questions in the ImplPlan.
2. Question-driven Clarification: ask focused, acceptance-oriented questions grouped by plan section. Maintain the Open Questions list in the ImplPlan and pre-link each item to a Decision Log placeholder.
3. Draft the Plan: complete the ImplPlan skeleton end-to-end (Purpose, Context, Plan of Work, Validation, Idempotence, etc.), calling out risks and dependencies.
4. Resolve Questions: as answers arrive, immediately update the ImplPlan—move items from Open Questions to the Decision Log with rationale; adjust Plan of Work and Acceptance accordingly.
5. Approval Gate: present the updated ImplPlan for approval. Do not implement until approved.
...
Example questions:
I reviewed src/main/lib/node-init-manager.ts to understand node initialization patterns.

    Where should this new feature live?
    a) src/main/ (main process — Node.js OK)
    b) src/renderer/ (renderer process — browser only)
    c) src/shared/ or src/lib/ (shared code — no Node.js)
    d) Other (specify)

    How should we handle errors?
    a) Return TRPCError at boundary
    b) Throw domain-specific error, catch at tRPC procedure
    c) Other (specify)

## How to use ImplPlans

When authoring an executable specification (ImplPlan), follow this guide to the letter. Be thorough in reading (and re-reading) source material to produce an accurate specification. When creating a spec, start from the skeleton and flesh it out as you do your research.

When implementing an executable specification (ImplPlan), do not prompt the user for "next steps"; simply proceed to the next milestone. Keep all sections up to date, add or split entries in the list at every stopping point to affirmatively state the progress made and next steps. Resolve ambiguities autonomously, and commit frequently.

When discussing an executable specification (ImplPlan), record decisions in a log in the spec for posterity; it should be unambiguously clear why any change to the specification was made. ImplPlans are living documents, and it should always be possible to restart from _only_ the ImplPlan and no other work.

## Design Standards

Before drafting, review `AGENTS.md` to ensure your design aligns with established patterns.
...
- Every ImplPlan must be fully self-contained. Self-contained means that in its current form it contains all knowledge and instructions needed for a novice to succeed.
- Every ImplPlan is a living document. Contributors are required to revise it as progress is made, as discoveries occur, and as design decisions are finalized. Each revision must remain fully self-contained.
- Every ImplPlan must enable a complete novice to implement the feature end-to-end without prior knowledge of this repo.
- Every ImplPlan must produce a demonstrably working behavior, not merely code changes to "meet a definition".
- Every ImplPlan must define every term of art in plain language or do not use it.
- Every ImplPlan must reference AGENTS.md conventions for this repository.
- Every ImplPlan must follow the design standards and coding conventions documented in AGENTS.md.
...
- **UI**: React + TailwindCSS v4 + shadcn/ui (in `src/ui/`)
- **Desktop**: Electron with tRPC over IPC (trpc-electron)
- **State**: Zustand

### Common Commands

    # Development
    bun dev                    # Start Electron dev server
    bun test                   # Run tests
...
For Electron work, remember:

- **Main process** (`src/main/`): Can use Node.js modules (fs, path, child_process, etc.)
- **Renderer process** (`src/renderer/`): Browser environment only, NO Node.js imports
- **Shared code** (`src/shared/`, `src/lib/`): NO Node.js modules — bundled for both processes
- **IPC**: All main↔renderer communication goes through tRPC (defined in `src/lib/trpc/routers/`)
...
Format and envelope are simple and strict. Each ImplPlan must be one markdown file. When you need to show commands, transcripts, diffs, or code, present them as indented blocks (4 spaces). Use two newlines after every heading, use # and ## and so on, and correct syntax for ordered and unordered lists.
...
Anchor the plan with observable outcomes. State what the user can do after implementation, the commands to run, and the outputs they should see. Acceptance should be phrased as behavior a human can verify ("after running `bun dev`, the Electron app opens and the new workspace sidebar is visible") rather than internal attributes ("added a WorkspaceSidebar component"). If a change is internal, explain how its impact can still be demonstrated.
...
**Example milestone structure:**
    ...
    ### Milestone 1: tRPC Procedure Foundation

    This milestone establishes the tRPC procedure for workspace operations.
    ...
    Scope:
    - Define procedure in src/lib/trpc/routers/workspace/create-workspace.ts
    - Add to workspace router in src/lib/trpc/routers/workspace/index.ts
    - Add unit tests for the procedure

    Acceptance:
        bun test src/lib/trpc/routers/workspace/create-workspace.test.ts
        # Expected: All tests pass

    Verify before proceeding: Types compile, tests pass, procedure logs correctly.
...
## Skeleton of a Good ImplPlan
    ...
    For this Caspian codebase, always mention:
    - Which process is affected (main, renderer, or both)
    - Which key directories are involved (src/main/, src/renderer/, src/lib/trpc/, src/shared/, etc.)
    - Any tRPC routers if adding new IPC procedures
    ...
    Launch the app:
        bun dev
        # Electron app opens, navigate to [feature], observe [behavior]
    ...
    For tRPC procedures, define in src/lib/trpc/routers/<domain>/:
...
If you follow the guidance above, a single, stateless agent -- or a human novice -- can read your ImplPlan from top to bottom and produce a working, observable result. That is the bar: SELF-CONTAINED, SELF-SUFFICIENT, NOVICE-GUIDING, OUTCOME-FOCUSED.

When you revise a plan, you must ensure your changes are comprehensively reflected across all sections, including the living document sections, and you must write a note at the bottom of the plan describing the change and the reason why. ImplPlans must describe not just the what but the why for almost everything.

## Plan Lifecycle

ImplPlans have a defined lifecycle that keeps the plans folders clean and provides a historical record of completed work.
